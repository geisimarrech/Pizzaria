...
(function ($) {
    $.fn.addTemplateSetup(function () {
        this.find('.mini-menu').css({
            opacity: 0
        }).parent().hover(function () {
            $(this).children('.mini-menu').stop(true).animate({
                opacity: 1
            });
        }, function () {
            $(this).children('.mini-menu').css('display', 'block').stop(true).animate({
                opacity: 0
            }, {
                'complete': function () {
                    $(this).css('display', '');
    ...
    $(document).ready(function () {
        ...
        if (!$(document.body).hasClass('special-page')) {
            ...
            function convertDropLists() {
                $(this).find('.result-block .small-files-list').accessibleList({
                    moreAfter: false
                });
                    ...
                    if ($(window).scrollTop() > controlBarPos) {
                        if (!controlBar.hasClass('fixed')) {
                            ...
                            $('#notifications').animate({
                                'top': controlBar.outerHeight() + notificationsTop
                            });
                        }
                    } else {
                        if (controlBar.hasClass('fixed')) {
...
(function ($) {
    $.modal = function (options) {
        ...
        if (settings.closeButton) {
            $('<ul class="action-tabs right"><li><a href="#" title="Close window"><img src="images/icons/fugue/cross-circle.png" width="16" height="16"></a></li></ul>').prependTo(win).find('a').click(function (event) {
                event.preventDefault();
